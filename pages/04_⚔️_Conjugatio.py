...
with practice_tabs[0]:
    ...
    with get_session() as session:
        ...
        with col1:
            if st.button("✅ Verificar", use_container_width=True, key="verify_guided"):
                # Save user answers
                st.session_state.user_conjugation_answers = {}
                for i in range(1, 4):
        ...
        with col2:
            if st.button("🔄 Limpiar", use_container_width=True, key="clear_guided"):
                st.session_state.show_conjugation_answers = False
                st.session_state.user_conjugation_answers = {}
                st.rerun()
        
        with col3:
            if st.button("🎲 Nuevo Verbo", use_container_width=True, key="new_verb_guided"):
                st.session_state.current_verb = random.choice(verbs)
                st.session_state.show_conjugation_answers = False
                st.session_state.user_conjugation_answers = {}
                st.rerun()
...
with practice_tabs[1]:
    ...
    with get_session() as session:
        ...
        # Select random verb
        if 'current_free_verb' not in st.session_state:
            st.session_state.current_free_verb = random.choice(filtered_verbs)
            
        # Check if current verb matches filters (if conjugation changed)
        current_verb = st.session_state.current_free_verb
        if current_verb not in filtered_verbs:
             st.session_state.current_free_verb = random.choice(filtered_verbs)
             current_verb = st.session_state.current_free_verb
        ...
        with col3:
            if st.button("🎲 Nuevo Verbo", use_container_width=True, key="new_verb_free"):
                st.session_state.current_free_verb = random.choice(filtered_verbs)
